...
def terrain_to_str(terrain: list[list[Terrain]]) -> str:
    res = ""
    for row in terrain:
        for cell in row:
            res += cell.value
...
def parse(input_str) -> list[list[Terrain]]:
    print("parse")
...
def simulate_sand(
    terrain: list[list[Terrain]], sand: list[tuple[int, int]]
    ...
    for col, row in sorted(
        ...
        while len(terrain) <= row_below:
            terrain.append([Terrain.AIR for _ in range(col + 1)])
...
def compute(input_str: str) -> str:
    ...
    while i < 1000:
        ...
        terrain, sand = simulate_sand(terrain, sand)
        for row, _ in sand:
            if row >= 500:
                break
        # print(terrain_to_str(terrain))
